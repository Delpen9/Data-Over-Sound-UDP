...
# data = [np.int64(i) if i > 1/3*max_value else 0 for i in data]
...
max_value_raw_data = np.amax(data)
...
# 0 --> 1 = distance of 250
peaks_teager, _ = find_peaks(post_processed, None, 0, 1, 1/20*max_value, [1.25, 2])
peak_first = peaks_teager[0]
peak_last = peaks_teager[len(peaks_teager) - 1]

peaks, _ = find_peaks(data, None, 0, 1, 1/400*max_value_raw_data, [1, 2.9])
peak_dimensions = peak_widths(data, peaks, rel_height=0.5)
...
print(post_processed_data_chunk)

post_processed_data_chunk = [floor(np.mean(i)) for i in post_processed_data_chunk]

print(post_processed_data_chunk)

post_processed_data = [1 for i in data]
for i in range(len(data)):
    if i not in post_processed_data_chunk:
        post_processed_data[i] = 0

post_processed_data[0:peak_first] = [0]*len(post_processed_data[0:peak_first])
post_processed_data[peak_last + 1:len(post_processed_data)] = [0]*len(post_processed_data[peak_last + 1:len(post_processed_data)])

# Time for Plots
time = np.arange(1, len(data) + 1)
time_first_teager = np.arange(1, len(teager_first_data) + 1)
time_second_teager = np.arange(1, len(teager_second_data) + 1)
time_post_processed = np.arange(1, len(post_processed_data) + 1)
